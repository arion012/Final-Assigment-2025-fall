...
In this task I would like to build a supervised learning algorithm which helps adventurers in D&D to determine a monsters cr based on the stats it has. So the adventuring party can prepare accordingly.
...
In the first graph we can see that there is a correlational tendency between armor class (ac) and crit rating (cr) however since there are just a few cases in the low ac, cr and the high counterparts they hacve a strong impact on the tendency.

We can see on the bar graph, that most creatures fall in the medium and large category. Beasts, monstrosities and dragons are almost represented in every category. The gargantuan category is dominated by dragons and no beasts can be found in the medium or small category.  
...
ell, its seems that the rsquare is 0.678 which is pretty high but I think it could be improved still, with some other variable. And the Rmse is pretty high 3.81 considering Cr goes up to 30 so the model is amiss with more than 10% on average. 
...
#pretty high AIC which means that the model doesn't not fit that well, and there is a high value of errors also but in itself we cant use this information just now, but when we build the second model. 
...
```{r, include=TRUE}
res <- aov(hp_number ~ size, data = Data_modified) 

#checking whether there is a difference between HP based on the size category, there might be an interaction going on between these two predictors. 

summary(res) 

#its seems like its a certain possibility
...
We managed to assert a supervised algorithm which does its job fairly well on guessing cr of monsters, which are not some outlying cases. But with an 1.23 rmse the mistakes are quite low and rsq is quite high (9.50) explaining a large portion of the residuals.
I hope this algorithm will be somewhat useful for future adventurers aiding them with guessing cr of monsters. 